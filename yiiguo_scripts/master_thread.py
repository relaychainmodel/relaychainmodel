...
demo:
python yiiguo_scripts/master.py --group_num=1 --node_num=4 --init=n
nohup python yiiguo_scripts/master.py --group_num=5 --node_num=4 --init_config=n --init_progress=y 2>&1 >yiiguo_scripts/log/master.out &
"""
import enum
import json
import threading
...
def get_tm_groups(group_num: int) -> queue.Queue:
    ...
    groups = queue.Queue(maxsize=group_num)
...
def start(groups: queue.Queue, progress_id: int, output_dir: str, arg_item: dict):
    ...
    sender = sendtx.Sender(
        url="10.21.4.37:{}".format(tm_group[1]+1),
        expargs=exp_arg,
        blockall=BLOCKALL,
    ...
    output_file = sender.start()
    get_log(tm_group, sender.get_expkey(), output_file)
