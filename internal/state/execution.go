...
func (blockExec *BlockExecutor) CreateProposalBlock( // [log by yiiguo] 创建提案区块
	height int64,
	state State, commit *types.Commit,
	proposerAddr []byte,
) (*types.Block, *types.PartSet) {

	maxBytes := state.ConsensusParams.Block.MaxBytes // [log by yiiguo] 默认21MB
	...
	blockExec.logger.Info("yiiguo block metrics", "maxDataBytes", strconv.FormatInt(maxDataBytes, 10))
...
func (blockExec *BlockExecutor) Commit(
	...
	// [log by yiiguo] remove txs from mempool
	// Update mempool.
	...
	content, error := ioutil.ReadFile("./txs_in_cs")
	if error != nil {
		blockExec.logger.Error("yiiguo readfile_after_update", "error", error.Error())
		panic(error)
	}
	infos := strings.Split(string(content), "/")
	error = ioutil.WriteFile("./txs_in_cs", []byte(fmt.Sprintf("%s/1", infos[0])), 0644)
	if error != nil {
		blockExec.logger.Error("yiiguo writefile_after_update", "error", error.Error())
		panic(error)
	}
