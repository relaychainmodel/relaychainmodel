...
func (txmp *TxMempool) ReapMaxBytesMaxGas(maxBytes, maxGas int64) types.Txs {
	txmp.logger.Info("yiiguo ReapMaxBytesMaxGas")
	txmp.mtx.RLock()
	defer txmp.mtx.RUnlock()
	...
	txs := make([]types.Tx, 0, numtxs)
	max_num := 50 // [log by yiiguo] max number txs picked each time
	txmp.logger.Info("yiiguo txmempool metrics", "numtxs", strconv.Itoa(numtxs))
	for txmp.priorityIndex.NumTxs() > 0 {
		wtx := txmp.priorityIndex.PopTx()
		txs = append(txs, wtx.tx)
		wTxs = append(wTxs, wtx)
		size := types.ComputeProtoSizeForTxs([]types.Tx{wtx.tx})
		// txmp.logger.Info("yiiguo transaction metrics", "txdatasize", strconv.FormatInt(size, 10))
		...
		// [log by yiiguo] limit the max number of txs in one block
		if len(txs) >= max_num {
			return txs[:max_num]
		}
	}
...
func (txmp *TxMempool) Update(
	...
	if txmp.Size() > 0 {
		if txmp.config.Recheck {
			...
			// DELAY := (50 * time.Millisecond).Nanoseconds()
			// MAXWAIT := (100 * time.Millisecond).Nanoseconds()
			// startTime := time.Now().Local().UnixNano()
			// lasttime := startTime
			// lastsize := txmp.Size()
			// for {
			// 	tempsize := txmp.Size()
			// 	temptime := time.Now().Local().UnixNano()
			// 	if MAXWAIT < temptime-startTime {
			// 		break // 不要一直等待, 最多等待 MAXWAIT 时间
			// 	}
			// 	if tempsize != lastsize {
			// 		lasttime = temptime
			// 		lastsize = tempsize
			// 	} else {
			// 		if DELAY < temptime-lasttime {
			// 			break
			// 		}
			// 	}
			// 	tmtime.OnlyDelay(2 * time.Millisecond)
			// }
			// txmp.logger.Info("yiiguo before notifyTxsAvailable", "txnum", txmp.Size())
