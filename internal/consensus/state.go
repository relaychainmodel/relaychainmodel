...
type State struct { // [log by yiiguo] 共识状态数据结构
	service.BaseService
	...
	// synchronous pubsub between consensus state and reactor. [log by yiiguo] 同步共识和reactor之间的订阅消息
...
// receive routines.
func (cs *State) OnStart() error { // [log by yiiguo] 共识启动主入口
...
func (cs *State) startRoutines(maxSteps int) { // [log by yiiguo] 没有用到该函数，所以receiveRoutine只在onstart()中进入
...
func (cs *State) newStep() {
	...
	if cs.eventBus != nil {
		...
		cs.evsw.FireEvent(types.EventNewRoundStepValue, &cs.RoundState) // [log by yiiguo] 在这里把消息发布给已经订阅过的reactor，然后reactor广播出去
	}
}
...
func (cs *State) receiveRoutine(maxSteps int) { // [log by yiiguo] the main go routines
	...
	for {
		...
		case mi = <-cs.internalMsgQueue: // [log by yiiguo] 处理内部事件
...
func (cs *State) handleMsg(mi msgInfo) { // [log by yiiguo] 提案消息，区块消息，投票消息
	cs.mtx.Lock()
	defer cs.mtx.Unlock()
	...
	case *BlockPartMessage: // [log by yiiguo] 理解：接收区块不是接收一整个区块，而是接收一个个的碎片然后再组装
	...
	case *VoteMessage: // [log by yiiguo] 处理投票消息
...
func (cs *State) handleTxsAvailable() {
	if cs.Step == cstypes.RoundStepNewRound {
		CONSTDELAY := 50 * time.Millisecond
		starttime := time.Now().Local().UnixMilli()
		cs.Logger.Info("yiiguo handleTxsAvailable before delay", "starttime", starttime)
		for {
			tmtime.OnlyDelay(2)
			now := time.Now().Local().UnixMilli()
			if CONSTDELAY.Milliseconds() < now-starttime {
				cs.Logger.Info("yiiguo handleTxsAvailable after delay", "endtime", now)
				break
			}
		}
	}
...
func (cs *State) enterNewRound(height int64, round int32) { // [log by yiiguo] 进入新一轮
	...
	if cs.Round < round {
		...
		validators.IncrementProposerPriority(tmmath.SafeSubInt32(round, cs.Round)) // [log by yiiguo] 提升validator的优先级
	}
...
func (cs *State) enterPropose(height int64, round int32) { // [log by yiiguo] 进入propose阶段
	logger := cs.Logger.With("height", height, "round", round)

	_ = ioutil.WriteFile("./txs_in_cs", []byte("0/0"), 0644)
	logger.Info("yiiguo writefile_txs_init", "timestamp", time.Now().Local().UnixNano())
	...
	defer func() {
		...
		logger.Info("fudelay before enterPropose")
		delayStart := time.Now().Local().UnixNano()
		cnt, randval := tmtime.FuDelay(0 * time.Millisecond)
		// cnt, randval := tmtime.OnlyDelay(500 * time.Millisecond)
		logger.Info(
			"fudelay of enterPropose",
			"cnt", cnt,
			"randval", randval,
			"cost", time.Now().Local().UnixNano()-delayStart,
		)

		cs.newStep() // [log by yiiguo] 在这里广播 proposal 事件
		if proposeStartTime != 0 {
			// [log by yiiguo] 这里是有误的, 不应该在这里计算 propose 的时间
			logger.Info("yiiguo time cost", "propose", time.Now().Local().UnixNano()-int64(proposeStartTime))
		}
	...
	logger.Info("yiiguo timestamp", "enterpropose", time.Now().Local().UnixNano())
	if cs.isProposer(address) { // [log by yiiguo] 判断是否是proposer
		logger.Debug(
			"propose step; our turn to propose",
			"proposer", address,
		)
		proposeStartTime = int(time.Now().Local().UnixNano())
...
func (cs *State) defaultDecideProposal(height int64, round int32) {
	...
	if err := cs.privValidator.SignProposal(ctx, cs.state.ChainID, p); err == nil {
		...
		cs.sendInternalMessage(msgInfo{&ProposalMessage{proposal}, ""}) // [log by yiiguo] 发送proposal消息，由 line: 864 - channel.internalMsg 接收
...
func (cs *State) enterPrevote(height int64, round int32) {
	...
	prevoteStartTime := time.Now().Local().UnixNano()
	defer func() {
		// Done enterPrevote:
		cs.updateRoundStep(round, cstypes.RoundStepPrevote)
		cs.newStep()
		if prevoteStartTime != 0 {
			logger.Info("yiiguo time cost", "prevote", time.Now().Local().UnixNano()-int64(prevoteStartTime))
		}
	}()

	logger.Debug("entering prevote step", "current", fmt.Sprintf("%v/%v/%v", cs.Height, cs.Round, cs.Step))
	logger.Info("yiiguo timestamp", "enterPrevote", time.Now().Local().UnixNano())
...
func (cs *State) enterPrecommit(height int64, round int32) {
	...
	logger.Info("yiiguo timestamp", "enterPrecommit", time.Now().Local().UnixNano())
	precommitStartTime := time.Now().Local().UnixNano()

	defer func() {
		// Done enterPrecommit:
		cs.updateRoundStep(round, cstypes.RoundStepPrecommit)

		// [log by yiiguo] 增加网络广播延迟, 负指数分布
		logger.Info("fudelay before enterPrecommit")
		delayStart := time.Now().Local().UnixNano()
		cnt, randval := tmtime.FuDelay(780 * time.Millisecond)
		// cnt, randval := tmtime.OnlyDelay(500 * time.Millisecond)
		logger.Info(
			"fudelay of enterPrecommit",
			"cnt", cnt,
			"randval", randval,
			"cost", time.Now().Local().UnixNano()-delayStart,
		)
		...
		cs.newStep()
		if precommitStartTime != int64(0) {
			logger.Info("yiiguo time cost", "precommit", time.Now().Local().UnixNano()-precommitStartTime)
		}
	}()
...
func (cs *State) enterCommit(height int64, commitRound int32) {
	...
	logger.Info("yiiguo timestamp", "enterCommit", time.Now().Local().UnixNano())
	commitStartTime := time.Now().Local().UnixNano()
	...
	defer func() {
		...
		// [log by yiiguo] 增加网络广播延迟, 负指数分布
		logger.Info("fudelay before enterCommit")
		delayStart := time.Now().Local().UnixNano()
		cnt, randval := tmtime.FuDelay(1150 * time.Millisecond)
		// cnt, randval := tmtime.OnlyDelay(500 * time.Millisecond)
		logger.Info(
			"fudelay of enterCommit",
			"cnt", cnt,
			"randval", randval,
			"cost", time.Now().Local().UnixNano()-delayStart,
		)
		...
		logger.Info("yiiguo timestamp", "endCommit", time.Now().Local().UnixNano())
		if commitStartTime != int64(0) {
			logger.Info("yiiguo time cost", "commit", time.Now().Local().UnixNano()-commitStartTime)
		}
	}()
...
func (cs *State) addProposalBlockPart(msg *BlockPartMessage, peerID types.NodeID) (added bool, err error) {
	...
	if added && cs.ProposalBlockParts.IsComplete() {
		...
		txs_num := len(cs.ProposalBlock.Data.Txs)
		err = ioutil.WriteFile("./txs_in_cs", []byte(fmt.Sprintf("%d/0", txs_num)), 0644)
		if err != nil {
			cs.Logger.Error("yiiguo writefile_txs", "txs_num", txs_num)
			panic(err)
		}
		content, err := ioutil.ReadFile("./txs_in_cs")
		if err != nil {
			cs.Logger.Error("yiiguo test_readfile_txs", "txs_num", txs_num)
			panic(err)
		}
		cs.Logger.Info("yiiguo writefile_txs", "txs_num", txs_num, "readfile", content, "timestamp", time.Now().Local().UnixNano())
...
func (cs *State) tryAddVote(vote *types.Vote, peerID types.NodeID) (bool, error) {
	...
	if err != nil { // [log by yiiguo] 投票异常处理，包括validator的惩罚
		// If the vote height is off, we'll just ignore it,
		// But if it's a conflicting sig, add it to the cs.evpool. 处理冲突投票而不是重复投票
		// If it's otherwise invalid, punish peer. 其他无效投票则直接惩罚
		if voteErr, ok := err.(*types.ErrVoteConflictingVotes); ok {
			if cs.privValidatorPubKey == nil {
				return false, errPubKeyIsNotSet
			}
			// [log by yiiguo] 冲突判断标准：来自其他peer的验证者信息和本地验证者信息一致
...
func (cs *State) addVote(vote *types.Vote, peerID types.NodeID) (added bool, err error) {
	cs.Logger.Debug(
		...
		"vote_type", vote.Type, // [log by yiiguo] 只允许两种合法的Type: PrevoteType = 1; PrecommitType = 2;
