...
const (
	StateChannel       = p2p.ChannelID(0x20) // 32
	DataChannel        = p2p.ChannelID(0x21) // 33
	VoteChannel        = p2p.ChannelID(0x22) // 34
	VoteSetBitsChannel = p2p.ChannelID(0x23) // 35
...
func (r *Reactor) OnStart() error { // [log by yiiguo] 来自peer的网络信息处理
...
func (r *Reactor) broadcastNewValidBlockMessage(rs *cstypes.RoundState) {
	...
	case <-r.closeCh:
		return
	case r.stateCh.Out <- p2p.Envelope{ // [log by yiiguo] 向其他的peer广播消息
		Broadcast: true,
...
// upon receiving.
func (r *Reactor) subscribeToBroadcastEvents() { // [log by yiiguo] 订阅共识、区块、投票等事件，然后向其他peer发送
...
func (r *Reactor) gossipDataForCatchup(rs *cstypes.RoundState, prs *cstypes.PeerRoundState, ps *PeerState) {
	...
	if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
		...
		// [log by yiiguo] 发送 data part for catchup 前增加网络广播延迟
		// r.Logger.Info(
		// 	"fudelay before BlockPartForCatchup",
		// 	"height", prs.Height,
		// 	"round", prs.Round,
		// )
		// delayStart := time.Now().Local().UnixNano()
		// cnt, _ := selftime.FuDelay(300 * time.Millisecond)
		// r.Logger.Info(
		// 	"fudelay after BlockPartForCatchup",
		// 	"cnt", cnt,
	...
	for {
		...
		if rs.ProposalBlockParts.HasHeader(prs.ProposalBlockPartSetHeader) {
			if index, ok := rs.ProposalBlockParts.BitArray().Sub(prs.ProposalBlockParts.Copy()).PickRandom(); ok {
				...
				// [log by yiiguo] 发送 data part 前增加网络广播延迟
				// r.Logger.Info(
				// 	"fudelay before sendingBlockPart",
				// 	"height", prs.Height,
				// 	"round", prs.Round,
				// )
				// delayStart := time.Now().Local().UnixNano()
				// cnt, _ := selftime.FuDelay(300 * time.Millisecond)
				// r.Logger.Info(
				// 	"fudelay after sendingBlockPart",
				// 	"cnt", cnt,
				// 	"cost", time.Now().Local().UnixNano()-delayStart,
				// 	"height", prs.Height,
				// 	"round", prs.Round,
		...
		if rs.Proposal != nil && !prs.Proposal {
			// Proposal: share the proposal metadata with peer.
			{
				...
				// [log by yiiguo] 发送 proposal 前增加网络广播延迟
				// r.Logger.Info(
				// 	"fudelay before sendingProposal",
				// 	"height", prs.Height,
				// 	"round", prs.Round,
				// )
				// delayStart := time.Now().Local().UnixNano()
			...
			if 0 <= rs.Proposal.POLRound {
				...
				// [log by yiiguo] 发送 proposalPOL 前增加网络广播延迟
				// r.Logger.Info(
				// 	"fudelay before sendingPOL",
				// 	"height", prs.Height,
				// 	"round", prs.Round,
				// )
				// delayStart := time.Now().Local().UnixNano()
...
func (r *Reactor) pickSendVote(ps *PeerState, votes types.VoteSetReader) bool {
	if vote, ok := ps.PickVoteToSend(votes); ok {
		...
		// [log by yiiguo] 发送 vote 前增加网络广播延迟
		// r.Logger.Info(
		// 	"fudelay before pickSendVote",
		// 	"votetype", votes.Type(),
		// 	"height", votes.GetHeight(),
		// 	"round", votes.GetRound(),
		// )
		// delayStart := time.Now().Local().UnixNano()
		// cnt, _ := selftime.FuDelay(500 * time.Millisecond)
		// r.Logger.Info(
		// 	"fudely after pickSendVote",
		// 	"cnt", cnt,
		// 	"cost", time.Now().Local().UnixNano()-delayStart,
		// 	"votetype", votes.Type(),
		// 	"height", votes.GetHeight(),
...
func (r *Reactor) handleDataMessage(envelope p2p.Envelope, msgI Message) error {
	...
	case *tmcons.Proposal:
		// r.Logger.Info("yiiguo datachannel", "type", "proposal")
	...
	case *tmcons.ProposalPOL:
		// r.Logger.Info("yiiguo datachannel", "type", "proposalpol")
		ps.ApplyProposalPOLMessage(msgI.(*ProposalPOLMessage))

	case *tmcons.BlockPart:
		// r.Logger.Info("yiiguo datachannel", "type", "blockpart")
	...
	default:
		// r.Logger.Info("yiiguo datachannel", "type", "default")
...
func (r *Reactor) handleMessage(chID p2p.ChannelID, envelope p2p.Envelope) (err error) {
	// [log by yiiguo] 增加网络广播延迟, 300ms 的负指数分布
	// if chID == DataChannel {
	// 	r.Logger.Info("fudelay before handleMessage")
	// 	delayStart := time.Now().Local().UnixNano()
	// 	cnt, randval := selftime.FuDelay(100 * time.Millisecond)
	// 	r.Logger.Info(
	// 		"fudeplay of handleMessage",
	// 		"cnt", cnt,
	// 		"randval", randval,
	// 		"chID", chID,
	// 		"cost", time.Now().Local().UnixNano()-delayStart,
	// 		"evlp.from", envelope.From,
	// 	)
	// }
...
// the reactor is stopped, we will catch the signal and close the p2p Channel
// gracefully.
func (r *Reactor) processVoteCh() { // [log by yiiguo] 接收来自网络中的投票
	for {
		select {
