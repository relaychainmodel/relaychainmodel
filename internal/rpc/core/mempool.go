...
func (env *Environment) BroadcastTxCommit(ctx *rpctypes.Context, tx types.Tx) (*coretypes.ResultBroadcastTxCommit, error) {
	env.Logger.Info("yiiguo env_BroadcastTxCommit", "tx", tx.Key())
	resCh := make(chan *abci.Response, 1)
	err := env.Mempool.CheckTx(
		ctx.Context(),
		tx,
	...
	env.Logger.Info("yiiguo broadcastTxCommit_checkTx0")
	r := (<-resCh).GetCheckTx()
	...
	env.Logger.Info("yiiguo broadcastTxCommit_checkTx1")
	...
	env.Logger.Info("yiiguo broadcastTxCommit_checkTx2")
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				env.Logger.Info("yiiguo broadcastTxCommit_checkTx_continue")
...
func (env *Environment) NumUnconfirmedTxs(ctx *rpctypes.Context) (*coretypes.ResultUnconfirmedTxs, error) {
	...
	if err != nil {
		...
		if err != nil {
			...
			num, err := strconv.Atoi(string(infos[0]))
			if err != nil {
				tx_in_cs = -3
			} else {
				tx_in_cs = num
