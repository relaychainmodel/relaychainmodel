...
func (r *Router) OpenChannel(chDesc ChannelDescriptor, messageType proto.Message, size int) (*Channel, error) {
	...
	queue := r.queueFactory(size) // [log by yiiguo] 接收 peer 消息的队列
...
func (r *Router) receivePeer(peerID types.NodeID, conn Connection) error {
	for {
		chID, bz, err := conn.ReceiveMessage() // [log by yiiguo] 接收 peer 消息 internal/p2p/transport_mconn.go:411
		if err != nil {
			return err
		}

		r.channelMtx.RLock()
		queue, ok := r.channelQueues[chID] // [log by yiiguo] 接收 peer 消息
		...
		select {
		case queue.enqueue() <- Envelope{From: peerID, Message: msg}: // [log by yiiguo] 把消息放入队列中
