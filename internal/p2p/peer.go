...
func (p *peer) processMessages() {
	...
	for {
		chID, msg, err := p.conn.ReceiveMessage() // [log by yiiguo] internal/p2p/switch.go:747
		if err != nil {
			p.onError(err)
			return
		}
		reactor, ok := p.reactors[byte(chID)] // [log by yiiguo] 这里的 reactors 列表应该包括 consensus 中的 reactor
		if !ok {
