...
func (c *MConnection) OnStart() error {
	...
	go c.sendRoutine() // [log by yiiguo] 给 peer 发送消息的 routine
	go c.recvRoutine() // [log by yiiguo] 从 peer 接收消息的 routine
	return nil
}
...
func (c *MConnection) sendPacketMsg() bool {
	...
	// [log by yiiguo] 发送的总关口, 增加网络广播延迟, 从每个 channel 中取出数据来
	// c.Logger.Info("fudelay before connection.sendPacketMsg")
	// delayStart := time.Now().Local().UnixNano()
	// cnt, randval := selftime.FuDelay(35 * time.Millisecond)
	// c.Logger.Info(
	// 	"fudeplay of connection.sendPacketMsg",
	// 	"cnt", cnt,
	// 	"randval", randval,
	// 	"chID", leastChannel.desc.ID,
		...
		*/
		...
		case *tmp2p.Packet_PacketMsg:
			...
			if msgBytes != nil {
				...
				c.onReceive(channelID, msgBytes) // [log by yiiguo] 收完所有的bytes之后, 触发 onReceive 函数, 但是这里依旧是在串行接收 msg, 但是 channel 之间是并行的
			}
		default:
