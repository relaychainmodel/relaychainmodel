...
func (c *mConnConnection) onReceive(chID byte, payload []byte) { // [log by yiiguo] internal/p2p/conn/connection.go:677
	select {
...
func (c *mConnConnection) SendMessage(chID ChannelID, msg []byte) (bool, error) {
	...
	case <-c.closeCh:
		return false, io.EOF
	default:
		// [log by yiiguo] 增加网络广播延迟, 这里只是放到对应的队列里面去, 还称不上总关口
		// c.logger.Info("fudelay before transport_mconn.SendMessage")
		// delayStart := time.Now().Local().UnixNano()
		// cnt, randval := selftime.FuDelay(100 * time.Millisecond)
		// c.logger.Info(
		// 	"fudeplay of mConnConnection.SendMessage",
		// 	"cnt", cnt,
		// 	"randval", randval,
		// 	"chID", chID,
...
func (c *mConnConnection) ReceiveMessage() (ChannelID, []byte, error) {
	...
	case msg := <-c.receiveCh:
		// [log by yiiguo] 接收的总关口, 增加网络广播延迟, 和另外三个位置互斥, 添加网络延迟最根本的位置
		// c.logger.Info("fudelay before transport_mconn.ReceiveMessage")
		// delayStart := time.Now().Local().UnixNano()
		// cnt, randval := selftime.FuDelay(100 * time.Millisecond)
		// c.logger.Info(
		// 	"fudeplay of mConnConnection.ReceiveMessage",
		// 	"cnt", cnt,
		// 	"randval", randval,
		// 	"chID", msg.channelID,
		// 	"cost", time.Now().Local().UnixNano()-delayStart,
